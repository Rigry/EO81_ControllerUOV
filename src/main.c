...
#define  ALARM_SET		GPIOC->BSRR = GPIO_Pin_7
#define  ALARM_RESET	GPIOC->BRR = GPIO_Pin_7
...
#define  UF_RON			GPIOB->BSRR = GPIO_Pin_10
#define	 UF_ROFF		GPIOB->BRR = GPIO_Pin_10
...
void save_pars(void);
void get_pars(void);		//ДК: читает чтото (наработка ламп точно) из еепрома

// ДК интерфейс с внешним миром
void USART1_Init(void);
void USART1_TX_DMA_Init(void);
void USART1_RX_DMA_Init(void);
void StartDMAChannel5(unsigned int LengthBufer);
...
int main (void)
{
	...
	DeviceState.disp_upd=0;
	DeviceState.uf_on=0;
	DeviceState.uzg_on=0;
	...
	USART1_RX_DMA_Init();
	USART1_TX_DMA_Init();
	StartDMAChannel5(100);	//включаем на прием канал ДМА
	
	ALARM_SET; UZG_SET; UF_SET; //включаем светодиодные индикаторы - проверка испарвности индикаторов
	UF_ROFF; UZG_ROFF;			//отключаем реле УЗГ И УФ
	...
	DeviceState.disp_red=1; 	//Выставляем флаг обновления дисплея-вывести главный дисплей при запуске
	ALARM_RESET; UZG_RESET;UF_RESET;// отключаем индикаторы
	...
	{	
		...
		switch (kb) {
			...
			case LEFT_HOLD: { // УФ
				switch  (DeviceState.uf_on) {
					case 1: { //УФ включен - выключаем УФ
						UF_RESET;
						UF_ROFF;
						DeviceState.uf_on=0;
						DeviceState.disp_upd=1;
					...
					*/	///////////////////////////////////////////
					...
					break;
					case 0: { //УФ выключен- включаем УФ
						UF_SET;
						UF_RON;
						DeviceState.uf_on=1;
						DeviceState.disp_upd=1;
			...
			case RIGHT_HOLD: { // УЗГ
				switch (DeviceState.uzg_on) {
					case 1: { //УЗГ включен- выключаем УЗГ
						UZG_RESET;
						UZG_ROFF;
						DeviceState.uzg_on=0;
						DeviceState.disp_upd=1;
					...
					break;
					case 0: { //УЗГ выключен- включаем УЗГ
						UZG_SET;
						UZG_RON;
						DeviceState.uzg_on=1; 
						DeviceState.disp_upd=1; 
					}
					break;
				} //switch					 
		...
		if((RxCMDReady==true)&&(CMD_BUF_RX[0]==BASE_ADDR)) {
			switch ( CMD_BUF_RX[1])	{
				case GETSTATE: { // Запрос состояния установки
					...
					CMD_BUF_TX[2]=	( (DeviceState.uf_on<<7) | (DeviceState.uzg_on<<6) | (DeviceState.alarm_uf<<5) | (DeviceState.alarm_lamp<<4)
									  | (DeviceState.alarm_temp<<3) | (DeviceState.alarm_comm<<2)
									) & 0xFF;
				...
				case SETUFON: { // Включить УФ
					UF_SET;//Включить светодиод УФ
					UF_RON;//Включить реле УФ
					DeviceState.uf_on=1;//Ставим флаг УФ вкл.
					DeviceState.disp_upd=1;//Обновляем дисплей на установке
				...
				case SETUFOF: { // Выключить УФ
					UF_RESET;//выключить светодиод УФ
					UF_ROFF; //Выключить реле УФ
					DeviceState.uf_on=0;//Изменяем статус установки
					DeviceState.disp_upd=1;//Обновить дисплей на установке
				}
				break;
											
				case SETUZON: { // Включить УЗ
					UZG_SET;//Включить светодиод УЗГ
					UZG_RON;//Включить реле УЗГ
					DeviceState.uzg_on=1;//Обновить статус установки
					DeviceState.disp_upd=1;//Обновить дисплей
				}
				break;
											
				case SETUZOF: { // Выключить УЗ
					UZG_RESET;//Выключить светодиод уdзг
					UZG_ROFF;//выключить реле узг
					DeviceState.uzg_on=0;//Обновить статус установки
					DeviceState.disp_upd=1;//Обновить дисплей на установке
				}
				break;
				...
				case BLAMPS: { // Запрос списка аварийных ламп  
					...
					count=2; 
					//Запаковать и послать
					for(i=0;i<=EXP_BOARD;i++) { // перетряхиваем платы на предмет нерабочих ламп
				...
				case HCOUNT: { //Запрос наработки ламп установки
					...
					count=2; 
					//Запаковать и послать
					for(i = 0; i < TOTAL_LAMPS; i++) { // перетряхиваем всю наработку которую насчитали
		...
		//////////////////////////////////////////////////////////////////////////////////////////
		 if (DeviceState.uf_on && (sTimer == 150)) { //если включена УФ
			lamps=0;
			for(i = 0; i <= EXP_BOARD; i++) {
				lamps|=get_lamps(i);// получаем список ламп подключенных в цикле по или суммируем от плат расширения и затем проверяем
			}
			if (lamps!=0) { //если лампы не горят
				if (!(DeviceState.alarm_lamp)) { //если тревога не установлена -устанавливаем
					ALARM_SET;//сравнивать с 0x3ff - если хоть одна лампа не горит- зажигаем тревогу
					DeviceState.alarm_lamp=1;// 
					DeviceState.disp_upd=1;
				}
			} else {
				if(DeviceState.alarm_lamp) { //если установлен флаг Тревога ламп-гасим его
					DeviceState.alarm_lamp=0;
					DeviceState.disp_upd=1;
				}
			}		
			...
			if (UF_T_BOARD)	{	
				...
				if  (uf_level<uf_threshold) { //если уровень УФ меньше порогового уровня 
					if (!(DeviceState.alarm_uf)) { //если тревога не установлена -устанавливаем
						//если низкий уровень УФ- зажигаем тревогу
						DeviceState.alarm_uf=1;
						DeviceState.disp_upd=1;
						ALARM_SET;
					}
				} else { // если уровень УФ выше порогового на 5 процентов
					if (DeviceState.alarm_uf && (uf_level > (uf_threshold+5)) ) { //если установлен флаг Тревога Ур.УФ-гасим его
						DeviceState.alarm_uf=0;		//  
						DeviceState.disp_upd=1;		//  
					}
				}
				...
				if (temperatura>UF_OFF_TEMP) { //если температура больше порога срабатывания защиты
					if (!DeviceState.alarm_temp) { //если тревога не установлена 
						//  зажигаем тревогу
						DeviceState.alarm_temp=1;// устанавливаем  флаг температуры
						DeviceState.disp_upd=1;
						ALARM_SET;
					}
					...
					if ((DeviceState.alarm_temp)&&(temperatura<UF_ON_TEMP)) {
						DeviceState.alarm_temp=0;
						DeviceState.disp_upd=1;
						UF_SET;
						UF_RON;
						if (DeviceState.uzg_on==1) { //Если УЗГ был включен с кнопки,тогда надо восттановить 
							UZG_SET;//Включить светодиод УЗГ
							UZG_RON;//Включить реле УЗГ
						}
					}
			...
			if (!(DeviceState.alarm_lamp | DeviceState.alarm_uf | DeviceState.alarm_temp)) {
				ALARM_RESET;							
			} else {
				ALARM_SET; 
			}			
			sTimer=0;
		} // if (DeviceState.uf_on && (sTimer == 150))
		 
		if (!(DeviceState.uf_on)) {
			ALARM_RESET;//если установка выключена- выключаем тревогу
		}	 
		sTimer++;
		/////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if (DeviceState.disp_upd || DeviceState.disp_red) { // если выставлен флаг обновления дисплея-обновляем дисплей
			if (UF_T_BOARD) {
			...
			display_main_screen();
			DeviceState.disp_red=0;
			DeviceState.disp_upd=0; //Сбрасываем флаг готовности обновления дисплея
		}
...
void MBMasterWork(void)
{
	...
	switch (eSt) {
		...
		case Delay:
			Timer[MBDelay].PT = 50;
			Timer[MBDelay].IN = true;
			UpdateTimer(&(Timer[MBDelay]));
			if (Timer[MBDelay].Q) {
				Timer[MBDelay].IN = false;
				UpdateTimer(&(Timer[MBDelay]));
...
void StartDMAChannel5(unsigned int LengthBufer)
{
  ...
  DMA1_Channel5->CNDTR =  LengthBufer;          //????????? ?????????? ?????? ??? ??????
